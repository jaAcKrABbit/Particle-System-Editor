...
void AParticle::computeDynamics(vector<float>& state, vector<float>& stateDot, float deltaT)
{
	...
	// Calculate stateDot
	for (int i = 0; i < 3; i++) {
		m_stateDot[i] = m_state[i + 3];
	}
	//force / m
	for (int i = 3; i < 6; i++) {
		m_stateDot[i] = m_state[i + 3] / m_mass;
	}
	for (int i = 6; i < 11; i++) {
		m_stateDot[i] = m_state[i];
	}
}
...
void AParticle::updateState(float deltaT, int integratorType)
{
	...
	switch (integratorType)
	{
		...
		case RK2:
		{
			// Add your code here
			for (int i = 0; i < 6; i++) {
				m_state[i] = m_state[i] + deltaT / 2 * (m_state[i + 3] + m_state[i + 3] + deltaT * m_stateDot[i + 3]);
			}
			/*for (int i = 3; i < 6; i++) {
				m_state[i] = 
			}*/
