...
public:
	...
	void setGuide(AJoint& transform) { m_Guide = transform; }

	vector<vec3>& getState() { return m_state; }   // gets the agent guide state information
	void setState(vector<vec3>& state) { m_state = state; }

	vec3 getPosition()  { return  m_Guide.getGlobalTranslation(); return m_Pos0; }  // gets the global position of agent
    vec3 getDesiredVelocity()  { return m_Vdesired; }
    vec3 getVelocity()  { return m_Vel0;  }
	vec3 getOrientation()  { vec3 angle(0.0, m_state[ORI][_Y_], 0.0); return angle; }  // gets global orientation of agent
	vec3 getForce(RefFrameType type);
	vec3 getTorque(RefFrameType type);
...
public:
	...
	static double leaderFollowDistance;
	// new
	static double obstacleOffset;
